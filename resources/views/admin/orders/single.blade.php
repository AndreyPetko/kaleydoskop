...
@section('content')
    ...
    <section class="content-header">
        ...
        <form method="POST" action="/admin/orders/set-declaration-number">
            ...
            <div class="row mt">
                <div class="col-md-3">
                    <input type="text" class="form-control" placeholder="Введите номер декларации" name="number"
                           @if($order->declarationNumber) value="{{$order->declarationNumber}}" @endif>
                </div>
                <div class="col-md-3">
            ...
            <div class="box-body table-responsive no-padding info-table">
                <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="" name="status">
                                ...
                                <option value="Отправлен(Оплачен)"
                                        @if($order->status == 'Отправлен(Оплачен)') selected @endif>Отправлен(Оплачен)
                                </option>
                                <option value="Отправлен(Ожидает оплаты)"
                                        @if($order->status == 'Отправлен(Ожидает оплаты)') selected @endif>
                                    Отправлен(Ожидает оплаты)
                                </option>
                                <option value="Ожидает" @if($order->status == 'Ожидает') selected @endif>Ожидает
                                </option>
                                <option value="Выполнен" @if($order->status == 'Выполнен') selected @endif>Выполнен
                                </option>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="delivery_type" id="">
                                ...
                                @endif
                                <option value="sam" @if($order->delivery_type == 'sam') selected @endif>Самовывоз
                                </option>
                                <option value="kuryer" @if($order->delivery_type == 'kuryer') selected @endif >Курьер
                                </option>
                                <option value="nova poshta" @if($order->delivery_type == 'nova poshta') selected @endif>
                                    Новой почтой
                                </option>
                                <option value="ukr poshta" @if($order->delivery_type == 'ukr poshta') selected @endif>
                                    Укрпочтой
                                </option>
                                <option value="autolux" @if($order->delivery_type == 'autolux') selected @endif>
                                    Автолюкс
                                </option>
                            </select>
                        </td>
                        ...
                        <td>
                            <select name="payment_type" id="">
                                @if($order->fast == 1)
                                    <option disabled selected value="">Не указано</option>
                                @endif
                                <option value="nal" @if($order->payment_type == 'nal') selected @endif>Наличными
                                    курьеру
                                </option>
                                <option value="privat" @if($order->payment_type == 'privat') selected @endif >На карту
                                    Приват Банка
                                </option>
                                <option value="visa" @if($order->payment_type == 'visa') selected @endif>Оплата
                                    Visa/Mastercart
                                </option>
                                <option value="nalog" @if($order->payment_type == 'nalog') selected @endif>Наложенный
                                    платеж
                                </option>
                            </select>
                        </td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="form-control" name="adress" value="{{$order->address}}"></input>
                        </td>
                    </tr>
    ...
    <section class="content">
            ...
            <table class="table table-bordered single-order-table">
                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <td><input value="{{$product->product_count}}" name="count[{{$product->product_id}}]"></input>
                        </td>
        ...
        <div class="row mt">
            <div class="col-md-3">
                <button type="submit" class="btn btn-block btn-primary btn-flat add-product-button">Сохранить
                    изменения
                </button>
            </div>
            <div class="col-md-3">
                <a href="/admin/orders/print/{{$order->id}}">
                    <div class="btn btn-block btn-primary btn-flat add-product-button">Печать</div>
                </a>
            </div>
        </div>
    </section>
